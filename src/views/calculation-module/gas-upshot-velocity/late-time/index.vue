    ...
    >
      ...
      >
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="钻头下深(m):" prop="bitVolume">
              <el-input
                v-model="formModel.bitVolume"
                placeholder="请输入"
              ></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item
              label="钻头深度的对应迟到时间(min):"
              prop="bitLogTime"
            >
              <el-input
                v-model="formModel.bitLogTime"
                placeholder="请输入"
              ></el-input>
            </el-form-item>
          </el-col>
          ...
          <el-col :span="12">
            <el-form-item label="开泵时间:" prop="openPumpTime">
              <el-date-picker
                v-model="formModel.openPumpTime"
                type="datetime"
                placeholder="选择时间"
          ...
          <el-col :span="12">
            <el-form-item label="后效开始时间:" prop="aftereffectTime">
              <el-date-picker
                v-model="formModel.aftereffectTime"
                type="datetime"
                placeholder="选择时间"
                value-format="yyyy-MM-dd HH:mm:ss"
                style="width: 100%"
  ...
  methods: {
    ...
    async getTableData() {
      try {
        ...
        const { data, code, message } = await getLogTimeList(params);
        if (code === 200) {
    ...
    calculate() {
      this.$refs["formRef"]?.validate(async (valid) => {
        ...
        try {
          ...
          const { code, message, data } = await logTimeCalculate(params);
          if (code === 200) {
